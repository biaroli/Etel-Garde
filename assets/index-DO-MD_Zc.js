...
`,".","!","?",";"];for(let i=Ue;i>Ue*.7;i--)if(r.includes(s[i])){n=i+1;break}t.push(s.substring(0,n)),s=s.substring(n)}return t}addChunks(e){this.lastAddedCount=e.length,this.chunks.push(...e)}removeLastEmbedding(){this.lastAddedCount>0&&this.lastAddedCount<=this.chunks.length&&(this.chunks.splice(-this.lastAddedCount),this.lastAddedCount=0)}tokenize(e){const t=e.replace(/[\s\p{P}]/gu,"");if(t.length===0)return[];const s=[];for(let n=0;n<t.length-1;n++)s.push(t[n]+t[n+1]);for(let n=0;n<t.length;n++)s.push(t[n]);return s}termFrequency(e){const t=new Map;for(const s of e)t.set(s,(t.get(s)||0)+1);return t}bm25Score(e,t,s,n,r){let i=0;const o=this.termFrequency(e);for(const[c,l]of o){const p=r.get(c)||0,u=t.get(c)||0;if(u===0)continue;const d=u*(ba+1),h=u+ba*(1-va+va*s/n);i+=p*(d/h)}return i}computeIDF(e){const t=e.length,s=new Map;for(const r of e)for(const i of r.keys())s.set(i,(s.get(i)||0)+1);const n=new Map;for(const[r,i]of s)n.set(r,Math.log((t-i+.5)/(i+.5)+1));return n}search(e,t,s=0,n="",r=.7){if(this.chunks.length===0)return[];r=Math.max(0,Math.min(1,r));const i=this.chunks.map(f=>this.cosineSimilarity(e,f.embedding));let o;if(r<1&&n.length>0){const f=this.tokenize(n),R=this.chunks.map(b=>this.tokenize(b.text)),g=R.map(b=>this.termFrequency(b)),k=R.map(b=>b.length),w=k.reduce((b,v)=>b+v,0)/k.length,y=this.computeIDF(g);o=this.chunks.map((b,v)=>this.bm25Score(f,g[v],k[v],w,y))}else o=new Array(this.chunks.length).fill(0);const c=this.minMaxNormalize(i),l=this.minMaxNormalize(o),p=this.chunks.map((f,R)=>{const g=r*c[R]+(1-r)*l[R],k=Math.max(0,s-(f.round||0)),w=Math.exp(-.003*k);return{text:f.text,score:g*w}});p.sort((f,R)=>R.score-f.score);const u=p.map(f=>f.score),d=u.reduce((f,R)=>f+R,0)/u.length,h=u.reduce((f,R)=>f+(R-d)**2,0)/u.length,_=Math.sqrt(h),m=Math.max(d+_,.1);return p.filter(f=>f.score>=m).slice(0,t)}minMaxNormalize(e){const t=Math.min(...e),n=Math.max(...e)-t;return n===0?e.map(()=>0):e.map(r=>(r-t)/n)}cosineSimilarity(e,t){if(e.length!==t.length)return 0;let s=0,n=0,r=0;for(let o=0;o<e.length;o++)s+=e[o]*t[o],n+=e[o]*e[o],r+=t[o]*t[o];const i=Math.sqrt(n)*Math.sqrt(r);return i===0?0:s/i}getCount(){return this.chunks.length}clear(){this.chunks=[],this.lastAddedCount=0}export(){return{chunks:this.chunks.map(e=>({text:e.text,embedding:e.embedding,round:e.round}))}}import(e){if(!e||!e.chunks){this.chunks=[];return}this.chunks=e.chunks.map(t=>({text:t.text,embedding:t.embedding,round:t.round??0})),this.lastAddedCount=0}}const Y=new Hn,yi="EtelGardeDB",Q="saves",bi=2,We="autosave";let gt="slot_0";const M={db:null,getCurrentSlotId(){return gt},setCurrentSlotId(a){gt=a},async init(){return new Promise((a,e)=>{const t=indexedDB.open(yi,bi);t.onerror=s=>{console.error("SaveService: IndexedDB error:",s),e("IndexedDB error")},t.onsuccess=s=>{this.db=s.target.result,a()},t.onupgradeneeded=s=>{const n=s.target.result;n.objectStoreNames.contains(Q)||n.createObjectStore(Q)}})},_buildSaveData(a,e){return{timestamp:Date.now(),history:O.exportHistory(),inventory:{player:j.playerInventory,companions:j.companionsData},userPersona:Pe.getUserPersona(),lastResponse:oe.state.lastRawResponse,lastUserInput:oe.state.lastUserInput,aiConfig:pe.config,vectorStoreData:Y.export(),slotId:a,slotName:e||`存档 ${a}`}},async saveSlot(a,e){this.db||await this.init();const t=this._buildSaveData(a,e);return new Promise((s,n)=>{const o=this.db.transaction([Q],"readwrite").objectStore(Q).put(t,`save_${a}`);o.onsuccess=()=>{s()},o.onerror=()=>{console.error(`SaveService: Save to slot ${a} Failed`),n("Save Failed")}})},async loadSlot(a){this.db||await this.init();const e=a==="legacy"?We:`save_${a}`;return new Promise((t,s)=>{const i=this.db.transaction([Q],"readonly").objectStore(Q).get(e);i.onsuccess=()=>{i.result?t(i.result):t(null)},i.onerror=()=>{console.error(`SaveService: Load slot ${a} Failed (key: ${e})`),s("Load Failed")}})},async deleteSlot(a){this.db||await this.init();const e=a==="legacy"?We:`save_${a}`;return new Promise((t,s)=>{const i=this.db.transaction([Q],"readwrite").objectStore(Q).delete(e);i.onsuccess=()=>{t()},i.onerror=()=>{console.error(`SaveService: Delete slot ${a} Failed (key: ${e})`),s("Delete Failed")}})},async listSlots(){return this.db||await this.init(),new Promise((a,e)=>{const n=this.db.transaction([Q],"readonly").objectStore(Q).openCursor(),r=[];n.onsuccess=i=>{const o=i.target.result;if(o){const c=o.key,l=o.value;if(c.startsWith("save_")||c===We){const p=c===We?"legacy":c.replace("save_","");r.push({slotId:p,slotName:l.slotName||(c===We?"旧存档 (自动迁移)":`存档 ${p}`),timestamp:l.timestamp||0})}o.continue()}else r.sort((c,l)=>l.timestamp-c.timestamp),a(r)},n.onerror=()=>{console.error("SaveService: List slots failed"),e("List Failed")}})},async saveGame(){await this.saveSlot(gt)},async loadGame(){this.db||await this.init();let a=await this.loadSlot(gt);return a||new Promise((e,t)=>{const r=this.db.transaction([Q],"readonly").objectStore(Q).get(We);r.onsuccess=()=>{r.result?e(r.result):e(null)},r.onerror=()=>{console.error("SaveService: Load Failed"),t("Load Failed")}})},async hasSave(){return this.db||await this.init(),(await this.listSlots()).length>0},createNewSlotId(){return`slot_${Date.now()}`}};class wa{constructor(e){ce(this,"config");this.config=e}isConfigured(){return!!(this.config.apiUrl&&this.config.apiKey&&this.config.model)}async listModels(){try{const e=this.config.apiUrl.replace(/\/+$/,"")+"/models",t=await fetch(e,{method:"GET",headers:{Authorization:`Bearer ${this.config.apiKey}`}});if(!t.ok)throw new Error(`HTTP ${t.status}`);return((await t.json()).data||[]).map(n=>n.id)}catch(e){return console.error("[EmbeddingService] Failed to list models:",e),[]}}async embed(e){if(!this.isConfigured())throw new Error("Embedding service not configured");const t=this.config.apiUrl.replace(/\/+$/,"")+"/embeddings",s=await fetch(t,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${this.config.apiKey}`},body:JSON.stringify({model:this.config.model,input:e,encoding_format:"float"})});if(!s.ok){const i=await s.text();throw new Error(`Embedding API error: HTTP ${s.status} - ${i}`)}const n=await s.json();if(!n.data||!Array.isArray(n.data))throw new Error("Unexpected embedding API response format");return n.data.sort((i,o)=>i.index-o.index).map(i=>i.embedding)}async embedSingle(e){return(await this.embed([e]))[0]}}const jn={fire:["#ff4500","#ff8c00","#ffd700","#ff0000"],ice:["#87ceeb","#e0f7fa","#ffffff","#00bfff"],lightning:["#e1f5fe","#ffffff","#7c4dff","#d500f9"],nature:["#4caf50","#81c784","#c8e6c9","#2e7d32"],dark:["#1a0033","#4a148c","#9c27b0","#000000"],holy:["#fff9c4","#ffffff","#ffeb3b","#ffd54f"],death:["#000000","#1b1b1b","#424242","#5c0000"],water:["#1565c0","#42a5f5","#90caf9","#00acc1"]},vi=a=>{const e=jn[a]||["#ffffff"];let t="circle";return a==="fire"&&(t="triangle"),a==="ice"&&(t="polygon"),a==="lightning"&&(t="line"),a==="holy"&&(t="star"),{particles:{color:{value:e},shape:{type:t,polygon:{sides:6}},opacity:{value:{min:.3,max:1},animation:{enable:!0,speed:2,sync:!1}},size:{value:{min:2,max:6},animation:{enable:!0,speed:10,sync:!1,startValue:"max",destroy:"min"}},move:{enable:!0,speed:15,direction:"none",random:!0,straight:!1,outModes:{default:"destroy"}}},interactivity:{events:{onHover:{enable:!1},onClick:{enable:!1}}},detectRetina:!0}};function wi(a,e,t){if(a==="none"||e==="none")return null;let s=1;t==="light"&&(s=.5),t==="heavy"&&(s=3);let n=vi(a);return e==="explosion"||e==="release"?(n.particles.number={value:200*s,density:{enable:!1}},n.particles.move.speed={min:20,max:60},n.particles.move.outModes={default:"destroy"},n.emitters={direction:"none",rate:{quantity:150*s,delay:.1},size:{width:0,height:0},position:{x:50,y:50},life:{duration:.1,count:1}},a==="dark"&&(n.particles.move.direction="inside",n.emitters.life.duration=.5,n.particles.move.speed={min:5,max:15})):e==="projectile"?(n.particles.number={value:100*s},n.particles.move.direction="right",n.particles.move.straight=!0,n.particles.move.speed=40,n.particles.move.trail={enable:!0,length:10,fillColor:"#000000"},n.emitters={direction:"right",rate:{quantity:5*s,delay:.01},position:{x:20,y:50}}):e==="summon"?(n.particles.number={value:150*s},n.particles.move.direction="top",n.particles.move.speed={min:3,max:10},n.emitters={direction:"top",position:{x:50,y:90},size:{width:40,height:10},rate:{quantity:10*s,delay:.1},life:{duration:2,count:1}}):e==="aoe"?(n.particles.number={value:300*s,density:{enable:!0,area:800}},n.particles.move.direction=a==="ice"||a==="water"?"bottom":"top",n.particles.move.speed={min:1,max:5},a==="lightning"&&(n.particles.move.speed={min:20,max:40},n.particles.shape.type="line")):(e==="start"||e==="channel")&&(n.particles.number={value:50*s},n.particles.move.direction="none",n.particles.move.speed=2,n.particles.move.attract={enable:!0,rotateX:600,rotateY:1200},n.emitters={direction:"none",spawnColor:{value:jn[a][0]},position:{x:50,y:50},rate:{quantity:2,delay:.1}}),n}class Yt{static async init(){window.tsParticles||console.warn("tsParticles hasn't loaded yet!")}static async playParticleVfx(e,t,s,n,r){if(e==="none"||t==="none")return Promise.resolve();const i=wi(e,t,s);if(!i)return Promise.resolve();if(i.emitters&&typeof i.emitters=="object"&&!Array.isArray(i.emitters)){const p=i.emitters;if(n&&(p.position={x:n.x,y:n.y}),t==="projectile"&&n&&r){const u=r.x>n.x?"right":"left";p.direction=u;const d=i.particles;d&&d.move&&(d.move.direction=u)}t==="explosion"&&r&&(p.position={x:r.x,y:r.y})}const o=t==="explosion"||t==="projectile"||t==="summon"?"vfx-overlay-canvas":"vfx-canvas",c=await window.tsParticles.load(o,i);let l=1500;return s==="light"&&(l=800),s==="heavy"&&(l=2500),(t==="aoe"||t==="summon")&&(l+=1e3),new Promise(p=>{setTimeout(()=>{c&&c.destroy(),p()},l)})}static async playScreenFx(e,t){const s=document.getElementById("app-root"),n=document.getElementById("screen-fx");if(!s||!n)return Promise.resolve();let r=0;if(e==="screen_shake"){const i=t==="strong"?"screen-shake-strong":"screen-shake-light";s.classList.add(i),r=t==="strong"?1e3:500,setTimeout(()=>s.classList.remove(i),r)}if(e==="screen_flash"){const i=t==="red"?"screen-flash-red":"screen-flash-white";n.classList.add(i),r=t==="red"?600:800,setTimeout(()=>n.classList.remove(i),r)}return r===0?Promise.resolve():new Promise(i=>setTimeout(i,r*.8))}}const oe={assetsConfig:{baseUrl:Tt.baseUrl,get characterMap(){return Vs},get backgroundMap(){return Re}},promptBuilder:new _i,elements:{},state:{dialogueLines:[],currentIndex:-1,spriteSlots:[{name:null,url:null,pos:"far_left"},{name:null,url:null,pos:"left"},{name:null,url:null,pos:"center"},{name:null,url:null,pos:"right"},{name:null,url:null,pos:"far_right"}],previousPositions:{},lastSpeaker:null,isTyping:!1,typewriterTimeout:null,isAutoPlaying:!1,autoPlayTimeout:null,isRequestingAI:!1,lastRawResponse:"",lastUserInput:"",isRegenerating:!1},_getRandomAssetUrl(a,e){if(!a)return null;let t=null;if(e[a]&&Array.isArray(e[a])&&e[a].length>0)t=e[a];else{const s=Object.keys(e);for(let n=0;n<s.length;n++){const r=s[n];if((a.includes(r)||r.includes(a))&&e[r]&&Array.isArray(e[r])&&e[r].length>0){t=e[r];break}}}if(t){const s=t[Math.floor(Math.random()*t.length)];return s.startsWith("http")?s:Tt.baseUrl+encodeURI(s)}return null},getSpriteUrl(a){return this._getRandomAssetUrl(a,Vs)},init(){if(this.elements={container:document.getElementById("galgame-container"),bg:document.getElementById("galgame-bg"),sprites:[document.getElementById("sprite-1"),document.getElementById("sprite-2"),document.getElementById("sprite-3"),document.getElementById("sprite-4"),document.getElementById("sprite-5")],dialogueBox:document.getElementById("dialogue-box"),speakerName:document.getElementById("speaker-name"),dialogueText:document.getElementById("dialogue-text"),autoplayBtn:document.getElementById("galgame-autoplay-btn"),prevBtn:document.getElementById("galgame-prev-btn"),historyLog:document.getElementById("galgame-history-log"),chatBtn:document.getElementById("chat-btn"),chatModal:document.getElementById("chat-modal"),chatInput:document.getElementById("chat-input"),sendChatBtn:document.getElementById("send-chat-btn"),fullscreenBtn:document.getElementById("fullscreen-btn"),dialogueSpinner:document.getElementById("dialogue-spinner")},Yt.init(),this.elements.fullscreenBtn&&this.elements.fullscreenBtn.addEventListener("click",()=>{document.fullscreenElement?document.exitFullscreen():document.documentElement.requestFullscreen()}),this.elements.chatBtn&&this.elements.chatModal&&this.elements.chatBtn.addEventListener("click",()=>{this.elements.chatModal.style.display="flex",this.elements.chatModal.classList.add("visible"),setTimeout(()=>this.elements.chatInput.focus(),50)}),this.elements.chatModal){const e=this.elements.chatModal.querySelector(".modal-close");e&&e.addEventListener("click",()=>{this.elements.chatModal.style.display="none",this.elements.chatModal.classList.remove("visible"),this.state.isRegenerating=!1}),this.elements.chatModal.addEventListener("contextmenu",t=>{t.preventDefault(),t.stopPropagation(),this.elements.chatModal.style.display="none",this.elements.chatModal.classList.remove("visible"),this.state.isRegenerating=!1})}const a=document.getElementById("regenerate-btn");a&&a.addEventListener("click",()=>{this.regenerateLastResponse()}),this.elements.sendChatBtn&&this.elements.sendChatBtn.addEventListener("click",async()=>{await this.handleChatSubmit()}),this.elements.container&&(this.elements.container.addEventListener("click",e=>{e.target.closest(".galgame-ui-text-btn")||this.elements.historyLog.classList.contains("visible")||(W.playSound("left"),this.advanceDialogue())}),this.elements.container.addEventListener("wheel",e=>{this.elements.historyLog.classList.contains("visible")||e.deltaY<0&&this.toggleHistoryLog(!0)},{passive:!0})),this.elements.historyLog&&this.elements.historyLog.addEventListener("click",e=>{const s=e.target.closest(".history-entry");if(s){const n=parseInt(s.dataset.index||"-1");!isNaN(n)&&n>=0&&(this.jumpToDialogue(n),W.playSound("left"))}}),this.elements.autoplayBtn&&this.elements.autoplayBtn.addEventListener("click",e=>{e.stopPropagation(),this.toggleAutoPlay()}),this.elements.prevBtn&&this.elements.prevBtn.addEventListener("click",e=>{e.stopPropagation(),this.previousDialogue()})},getValue(a,e){var t,s;return((s=(t=e.querySelector(a))==null?void 0:t.textContent)==null?void 0:s.trim())||""},updateGameState(a){var m,f,R;const e=this.getValue("inventory",a),t=this.getValue("time",a),s=this.getValue("wealth",a),n=this.getValue("pow",a),r=parseInt(((m=s.match(/白金币\s*[:：]\s*(\d+)/))==null?void 0:m[1])||"0",10),i=s.match(/(?:^|[^白])金币\s*[:：]\s*(\d+)/),o=i?parseInt(i[1],10):0,c=parseInt(((f=s.match(/银币\s*[:：]\s*(\d+)/))==null?void 0:f[1])||"0",10),l=parseInt(((R=s.match(/铜币\s*[:：]\s*(\d+)/))==null?void 0:R[1])||"0",10);j.setPlayerData({items:e,time:t,wealth:s,pow:n,platinum:r,gold:o,silver:c,copper:l});const p=a.querySelectorAll("companions > companion"),u=Array.from(p).map(g=>({name:this.getValue("name",g)||"未知",affection:parseInt(this.getValue("affection",g)||"0"),status:this.getValue("status",g)||"正常",fight:this.getValue("fight",g)||"未知",bag:this.getValue("bag",g)||"",portrait:this.getSpriteUrl(this.getValue("name",g)||""),platinum:0,gold:0,silver:0,copper:0}));j.setCompanionsData(u),j.renderCompanionUI();const d=document.getElementById("task-body-wrapper"),h=a.querySelectorAll("events > event");if(d)if(d.innerHTML="",h.length>0){const g=Array.from(h).map(P=>({name:this.getValue("name",P)||"未知的契约",description:this.getValue("description",P)||"迷雾遮蔽了契约的内容..."}));let k='<div class="task-list-container">';g.forEach((P,D)=>{k+=`
                    ...
                    </div>`;d.innerHTML=k+w;const y=d.querySelectorAll(".task-list-item"),b=d.querySelector(".task-title"),v=d.querySelector(".task-description");y.forEach(P=>{P.addEventListener("click",()=>{if(P.classList.contains("active"))return;y.forEach(E=>E.classList.remove("active")),P.classList.add("active");const D=parseInt(P.dataset.taskIndex),L=g[D];b&&(b.textContent=L.name),v&&(v.textContent=L.description),W.playSound("left")})})}else d.innerHTML=`
